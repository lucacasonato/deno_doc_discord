...
export function nodeName(name: string, scope?: string[]): string {
  return (scope && scope?.length > 0 ? scope.join(".") + "." : "") + name;
}
  ...
  while (!done) {
    const node = flattend.find(
      (node) =>
        ...
        nodeName(node.name, node.scope) === nodeName(name, scope) &&
...
export function getLinkByScopedName(
  ...
  if (runtimeBuiltins) {
    const node = findNodeByScopedName(runtimeBuiltins, name, [], mustBe);
    if (node) {
      return {
        type: "builtin",
        href: `#${node.scope ? node.scope.join(".") + "." : ""}${node.name}`,
